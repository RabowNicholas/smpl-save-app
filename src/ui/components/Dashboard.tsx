...
export function Dashboard() {
  ...
  const handleConfirmRemove = async () => {
    ...
    try {
      // Check if it's a custom service or regular service
      const isCustomService = selectedCustomServices.some(cs => cs.id === serviceToRemove)
      
      let response;
      if (isCustomService) {
        response = await fetch(`/api/custom-services?userId=${user.id}&customServiceId=${serviceToRemove}`, {
          method: 'DELETE'
        })
      } else {
        response = await fetch(`/api/user-services?userId=${user.id}&serviceId=${serviceToRemove}`, {
          method: 'DELETE'
        })
      }
      ...
      if (isCustomService) {
        // Update custom service details
        setSelectedCustomServices(prev => prev.filter(service => service.id !== serviceToRemove))
      } else {
        // Update regular service details
        ...
        <div className="max-w-sm mx-auto text-center relative z-10">
          ...
          <div className="backdrop-blur-sm bg-slate-800/90 rounded-3xl p-8 shadow-2xl border border-indigo-900/40 ring-1 ring-purple-500/20 mb-8">
            ...
            <p className="text-indigo-200 text-lg font-medium mb-4">
              Select the services you want to save on
            </p>
            
            <p className="text-lg text-slate-300 mb-8">
              Add your services to start getting better rates
            </p>
          </div>
      ...
      <div className="max-w-md mx-auto relative z-10">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3">
            ...
            {/* Custom Services */}
            {selectedCustomServices.map((customService) => {
              const isRemoving = removingServices[customService.id]
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className="w-10 h-10 mr-3 rounded-lg flex items-center justify-center shadow-lg bg-emerald-600">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-300">Custom Service</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => handleRemoveServiceClick(customService.id)}
                        disabled={isRemoving}
                        className="w-8 h-8 bg-red-900/30 hover:bg-red-800/50 text-red-400 hover:text-red-300 rounded-lg transition-all duration-200 flex items-center justify-center border border-red-700/30 hover:border-red-600/50 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <ConfirmModal
          ...
          title="Remove Service"
          message={`Are you sure you want to remove ${selectedServiceDetails.find(s => s.id === serviceToRemove)?.name || selectedCustomServices.find(cs => cs.id === serviceToRemove)?.name || 'this service'} from your tracked services?`}
          confirmText="Remove"
          cancelText="Keep It"
