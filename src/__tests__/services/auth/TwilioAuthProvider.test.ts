...
describe('TwilioAuthProvider', () => {
  ...
  describe('sendVerificationCode', () => {
    it('should send verification code successfully', async () => {
      ...
      expect(mockVerify.v2.services).toHaveBeenCalledWith('test-verify-service-sid')
    ...
    it('should handle Twilio API errors', async () => {
      mockVerifyService.verifications.create.mockRejectedValue(new Error('Invalid phone number'))

      await expect(authProvider.sendVerificationCode(testPhone))
        .rejects.toThrow('Failed to send verification code: Invalid phone number')
    })
    ...
    it('should validate phone number format', async () => {
      await expect(authProvider.sendVerificationCode('invalid-phone'))
  ...
  describe('verifyCode', () => {
    ...
    it('should handle invalid verification code', async () => {
      ...
      await expect(authProvider.verifyCode(testPhone, testCode))
    ...
    it('should handle Twilio API errors during verification', async () => {
      mockVerifyService.verificationChecks.create.mockRejectedValue(new Error('Service unavailable'))
    ...
    it('should validate phone number format', async () => {
      await expect(authProvider.verifyCode('invalid-phone', testCode))
        .rejects.toThrow('Invalid phone number format')

      expect(mockVerifyService.verificationChecks.create).not.toHaveBeenCalled()
    })

    it('should validate code format', async () => {
      await expect(authProvider.verifyCode(testPhone, 'invalid-code'))
    ...
    it('should handle user creation errors', async () => {
      ...
      mockDatabaseClient.createUser.mockRejectedValue(new Error('Database error'))
